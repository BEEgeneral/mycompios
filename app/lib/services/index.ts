/**
 * Service Layer - Polsia-style
 * 
 * Based on Polsia's app/services/
 * 
 * Exports:
 * - taskService: Task CRUD + dispatch
 * - companyService: Context aggregation
 * - activityService: Logging + broadcast
 * - memoryService: Memory management
 * - reportService: Daily reports
 */

export * from './task-service'
export * from './company-service'  
export * from './activity-service'
export * from './memory-service'
export * from './report-service'
export * from './vector-service'
export * from './marketing-service'
export * from './agent-service'
export * from './user-service'
export * from './research-service'
export * from './orchestrator-service'
export * from './auth-service'
export * from './chat-service'
export * from './execute-service'
export * from './stripe-service'
export * from './auth-reset-service'
// login-service and register-service exports getTrialStatus which conflicts with auth-service
// export * from './login-service'
// export * from './register-service'
export * from './autonomous-service'
export * from './email-sequence-service'
